...
const std::vector<float> PAN_LIMIT = {-M_PI, M_PI}; // パン(水平回転)の可動範囲[rad]
const std::vector<float> TILT_LIMIT = {-M_PI/2, 70/180*M_PI}; // チルト(垂直回転)の可動範囲[rad]
const std::vector<float> INIT_POS = {0.0, M_PI/3.0}; // 初期位置[rad]
const std::vector<float> RAD_CORRECTION = {0,0}; // ラジアン補正値[rad]
// ********************************************************************************************************************
// クラスの定義 
// ********************************************************************************************************************
class PanTiltNode : public rclcpp::Node{
    ...
    public:
    PanTiltNode() : Node("pan_tilt_node"){
        RCLCPP_INFO(this->get_logger(), "pan_tilt_node is activated");
        // パラメータの宣言と取得
        auto parameter0 = this->declare_parameter<std::string>("color", "blue");
        auto parameter1 = this->declare_parameter<std::string>("device", "/dev/ttyUSB0");
        auto parameter2 = this->declare_parameter<int>("baudrate", 115200);
        auto parameter3 = this->declare_parameter<int>("id0", 6); // pan
        auto parameter4 = this->declare_parameter<int>("id1", 5); // tilt 1
        // auto parameter5 = this->declare_parameter<int>("id2", 6); // tilt 2
        ...
        auto timer_callback = [this]() -> void{
            ...
            present_position += RAD_CORRECTION[0];
            if (target_position[0] - present_position > DELTA_RAD){ // target_positionがpresent_positionより十分大きい場合はDELTA_RADだけ増加
                dxl_wb.goalPosition(id0, present_position + DELTA_RAD + RAD_CORRECTION[0], &log);
                ...
                dxl_wb.goalPosition(id0, target_position[0] + RAD_CORRECTION[0], &log);
            }
            // dxl_wb.getRadian(id1, &present_position);
            // present_position += RAD_CORRECTION[1];
            // if (target_position[1] - present_position > DELTA_RAD){
            //     dxl_wb.goalPosition(id1, present_position + DELTA_RAD - RAD_CORRECTION[1], &log);
            //     // dxl_wb.goalPosition(id2, -present_position - DELTA_RAD + RAD_CORRECTION[1], &log);
            // }else if (target_position[1] - present_position < -DELTA_RAD){
            //     dxl_wb.goalPosition(id1, present_position - DELTA_RAD - RAD_CORRECTION[1], &log);
            //     // dxl_wb.goalPosition(id2, -present_position + DELTA_RAD + RAD_CORRECTION[1], &log);
            // }else{
            //     dxl_wb.goalPosition(id1, target_position[1] - RAD_CORRECTION[1], &log);
            //     // dxl_wb.goalPosition(id2, -target_position[1] + RAD_CORRECTION[1], &log);
            // }
        };
        
        auto topic_callback = [this](const SetPosition::SharedPtr msg) -> void{
            if (msg->position_0 < PAN_LIMIT[0]){
                std::cout << "ID " << id0_ << " の目標値が可動域外です\n目標値を再設定します" << std::endl;
                target_position[0] = PAN_LIMIT[0];
            }else if(msg->position_0 > PAN_LIMIT[1]){
                std::cout << "ID " << id0_ << " の目標値が可動域外です\n目標値を再設定します" << std::endl;
                target_position[0] = PAN_LIMIT[1];
            }else{
                target_position[0] = msg->position_0;
            }

            // if (M_PI/2 - msg->position_1 < TILT_LIMIT[0]){
            //     std::cout << "ID " << id1_ << " の目標値が可動域外です\n目標値を再設定します" << std::endl;
            //     target_position[1] = TILT_LIMIT[0];
            // }else if(M_PI/2 - msg->position_1 > TILT_LIMIT[1]){
            //     std::cout << "ID " << id1_ << " の目標値が可動域外です\n目標値を再設定します" << std::endl;
            //     target_position[1] = TILT_LIMIT[1];
            // }else{
            //     target_position[1] = M_PI/2 - msg->position_1;
            // }
        };
        ...
        auto service_callback = [this](const std::shared_ptr<GetPosition::Request> request, std::shared_ptr<GetPosition::Response> response) -> void{
            float present_position;
            if(request->id == 0){
                dxl_wb.getRadian(id0, &present_position, &log);
            }else if(request->id == 1){
                dxl_wb.getRadian(id0, &present_position, &log);
                present_position = M_PI/2 - present_position;
            }
        ...
        subscriber =this->create_subscription<SetPosition>("set_pantilt", QOS_RKL10V, topic_callback);
        server = create_service<GetPosition>("get_pantilt", service_callback);
...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<PanTiltNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}