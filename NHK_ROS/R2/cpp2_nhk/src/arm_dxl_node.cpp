...
// 定数等の定義
// ********************************************************************************************************************
const float DELTA_RAD = 0.1; // 1周期あたりの変化量[rad]
const std::vector<float> ID0_LIMIT = {-20.0/180.0*M_PI, 65.0/180.0*M_PI}; // 第一関節の上限と下限{min, max}[rad]
const std::vector<float> ID1_LIMIT = {-65.0/180.0*M_PI, 65.0/180.0*M_PI}; // 第二関節の上限と下限{min, max}[rad]
const std::vector<float> INIT_POS = {1.1, -1.1}; // 初期位置[rad] ahead
// const std::vector<float> INIT_POS = {0.0, 0.0}; // 初期位置[rad] ahead
// ********************************************************************************************************************
// クラスの定義 
// ********************************************************************************************************************
class DynamixelNode : public rclcpp::Node{
    ...
    int id0_, id1_, id2_, id3_;
    // id0：第一関節の右側 id1：第一関節の左側 id2：第二関節の右側 id3：第二関節の左側
    std::string color = "";
    const char* log;
    ...
    DynamixelNode() : Node("dynamixel_node"){
        ...
        auto parameter1 = this->declare_parameter<std::string>("device", "/dev/U2D2-R2-1");
        auto parameter2 = this->declare_parameter<int>("baudrate", 115200);
        auto parameter3 = this->declare_parameter<int>("id0", 6);
        auto parameter4 = this->declare_parameter<int>("id1", 1);
        auto parameter5 = this->declare_parameter<int>("id2", 5);
        auto parameter6 = this->declare_parameter<int>("id3", 2);
        ...
        if(!this->get_parameter("id2", id2_)){
            std::cout << "id2 パラメータの取得に失敗" << std::endl;
        }else{
            std::cout << "id2: " << id2_ << std::endl;
        }
        id2 = (uint8_t)id2_;

        if(!this->get_parameter("id3", id3_)){
            std::cout << "id3 パラメータの取得に失敗" << std::endl;
        }else{
            std::cout << "id3: " << id3_ << std::endl;
        }
        id3 = (uint8_t)id3_;
        ...
        model_number = 0;
        dxl_wb.ping(id2, &model_number, &log);
        std::cout << "ID " << id2_ << " へPingを送信\n" << log << std::endl;
        if(model_number == PRO_H42_20_S300_R_A){
            std::cout << "検出されたモデル: PRO_H42_20_S300_R_A" << std::endl;
        }else if(model_number == MX_106_2){
            std::cout << "検出されたモデル: MX_106_2" << std::endl;
        ...
        model_number=0;
        dxl_wb.ping(id3, &model_number, &log);
        std::cout << "ID " << id3_ << " へPingを送信\n" << log << std::endl;
        if(model_number == PRO_H42_20_S300_R_A){
            std::cout << "検出されたモデル: PRO_H42_20_S300_R_A" << std::endl;
        }else if(model_number == MX_106_2){
            std::cout << "検出されたモデル: MX_106_2" << std::endl;
        ...
        std::cout << "ID " << id0_ << " を位置制御モードに設定\n" << log << std::endl;
        dxl_wb.setPositionControlMode(id1, &log);
        std::cout << "ID " << id1_ << " を位置制御モードに設定\n" << log << std::endl;
        dxl_wb.setPositionControlMode(id2, &log);
        std::cout << "ID " << id2_ << " を位置制御モードに設定\n" << log << std::endl;
        dxl_wb.setPositionControlMode(id3, &log);
        std::cout << "ID " << id3_ << " を位置制御モードに設定\n" << log << std::endl;

        dxl_wb.torqueOn(id0, &log);
        std::cout << "ID " << id0_ << " のトルクをオン\n" << log << std::endl;
        dxl_wb.torqueOn(id1, &log);
        std::cout << "ID " << id1_ << " のトルクをオン\n" << log << std::endl;
        dxl_wb.torqueOn(id2, &log);
        std::cout << "ID " << id2_ << " のトルクをオン\n" << log << std::endl;
        dxl_wb.torqueOn(id3, &log);
        std::cout << "ID " << id3_ << " のトルクをオン\n" << log << std::endl;
        ...
        auto timer_callback = [this]() -> void{
            ...
            // target 30, 50
            std::cout << "target:" << target_position[0] << ", " << target_position[1] << std::endl;
            dxl_wb.getRadian(id0, &present_position); // 60
            std::cout << "ID" << id0_ << ": " << present_position << std::endl;
            if (target_position[0] - present_position > DELTA_RAD){ // target_positionがpresent_positionより十分大きい場合はDELTA_RADだけ増加
                ...
                std::cout << "Set: " << present_position + DELTA_RAD << std::endl;
                ...
                std::cout << "Set: " << target_position[0] << std::endl;
            }
            dxl_wb.getRadian(id1, &present_position); // -60
            std::cout << "ID" << id1_ << ": " << present_position << std::endl;
            if (target_position[0] + present_position > DELTA_RAD){ // target_positionがpresent_positionより十分大きい場合はDELTA_RADだけ増加
                ...
                dxl_wb.goalPosition(id1, -target_position[0], &log);
                std::cout << "Set: " << -target_position[0] << std::endl;
            }
            dxl_wb.getRadian(id2, &present_position);
            std::cout << "ID" << id2_ << ": " << present_position << std::endl;
            if (target_position[1] - present_position > DELTA_RAD){
                dxl_wb.goalPosition(id2, present_position + DELTA_RAD, &log);
                std::cout << "Set: " << present_position + DELTA_RAD << std::endl;
            }else if (target_position[1] - present_position < -DELTA_RAD){
                dxl_wb.goalPosition(id2, present_position - DELTA_RAD, &log);
                std::cout << "Set: " << present_position - DELTA_RAD << std::endl;
            }else{
                dxl_wb.goalPosition(id2, target_position[1], &log);
                std::cout << "Set: " << target_position[1] << std::endl;
            }
            dxl_wb.getRadian(id3, &present_position);
            std::cout << "ID" << id3_ << ": " << present_position << std::endl;
            if (target_position[1] + present_position > DELTA_RAD){
                dxl_wb.goalPosition(id3, present_position - DELTA_RAD, &log);
                std::cout << "Set: " << present_position - DELTA_RAD << std::endl;
            }else if (target_position[1] + present_position < -DELTA_RAD){
                dxl_wb.goalPosition(id3, present_position + DELTA_RAD, &log);
                std::cout << "Set: " << present_position + DELTA_RAD << std::endl;
            }else{
                dxl_wb.goalPosition(id3, -target_position[1], &log);
                std::cout << "Set: " << -target_position[1] << std::endl;
            }
        };
        
        auto topic_callback = [this](const SetPosition::SharedPtr msg) -> void{
            if (msg->position_0 < ID0_LIMIT[0]){
                std::cout << "ID " << id0_ << " の目標値が可動域外です\n目標値を再設定します" << std::endl;
                target_position[0] = ID0_LIMIT[0];
            }else if(msg->position_0 > ID0_LIMIT[1]){
                std::cout << "ID " << id0_ << " の目標値が可動域外です\n目標値を再設定します" << std::endl;
                target_position[0] = ID0_LIMIT[1];
            }else{
                target_position[0] = msg->position_0;
            }

            if (msg->position_1 < ID1_LIMIT[0]){
                std::cout << "ID " << id2_ << " の目標値が可動域外です\n目標値を再設定します" << std::endl;
                target_position[1] = ID1_LIMIT[0];
            }else if(msg->position_1 > ID1_LIMIT[1]){
                std::cout << "ID " << id2_ << " の目標値が可動域外です\n目標値を再設定します" << std::endl;
                target_position[1] = ID1_LIMIT[1];
            }else{
                target_position[1] = msg->position_1;
            }
        };
        ...
        auto service_callback = [this](const std::shared_ptr<GetPosition::Request> request, std::shared_ptr<GetPosition::Response> response) -> void{
            ...
            if(request->id == 3 or request->id == 4){
        ...
        subscriber =this->create_subscription<SetPosition>("set_arm_position", QOS_RKL10V, topic_callback);
        server = create_service<GetPosition>("get_position", service_callback);
        rclcpp::sleep_for(1s); // dynamixel2秒待機
    ...
    ~DynamixelNode(){
        ...
        dxl_wb.torqueOff(id2, &log);
        std::cout << "ID " << (int)id2 << " のトルクをオフ\n" << log << std::endl;
        dxl_wb.torqueOff(id3, &log);
        std::cout << "ID " << (int)id3 << " のトルクをオフ\n" << log << std::endl;
    }
};
// ********************************************************************************************************************
// メイン関数
